        ...
        if (empty($emailTeste)) {
            ...
            try {
                $configs = [
                    ...
                    'smtp_de_email' => $_POST['smtp_de_email'] ?? '',
                    'smtp_de_nome' => $_POST['smtp_de_nome'] ?? 'Sistema de Chamada',
                    'smtp_seguranca' => $_POST['smtp_seguranca'] ?? 'tls',
                    'smtp_html' => $_POST['smtp_html'] ?? '1',
                ];
...
<div class="bg-white rounded-lg shadow-md p-6 max-w-3xl">
    <form method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="password" name="smtp_senha" value="<?= htmlspecialchars($configs['smtp_senha'] ?? '') ?>" 
                   placeholder="Senha do email" class="w-full border border-gray-300 rounded-lg px-4 py-2">
