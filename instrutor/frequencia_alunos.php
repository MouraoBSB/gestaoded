...
if ($userType === 'instrutor') {
    $cursos = $pdo->prepare("
        SELECT id, nome, ano 
        FROM cursos 
        WHERE instrutor_id = ? AND ativo = 1 
...
$cursoSelecionado = isset($_GET['curso_id']) ? (int)$_GET['curso_id'] : 0;
...
if ($cursoSelecionado) {
    $stmt = $pdo->prepare("
        SELECT a.id, a.nome, a.foto,
               ...
               ROUND((SUM(CASE WHEN p.presente = 1 THEN 1 ELSE 0 END) / COUNT(DISTINCT au.id)) * 100) as frequencia
        FROM alunos a
        INNER JOIN matriculas m ON a.id = m.aluno_id
        LEFT JOIN aulas au ON m.curso_id = au.curso_id
        LEFT JOIN presencas p ON au.id = p.aula_id AND a.id = p.aluno_id
        WHERE m.curso_id = ? AND a.ativo = 1
        GROUP BY a.id
        ORDER BY a.nome
    ");
    $stmt->execute([$cursoSelecionado]);
...
<div class="bg-white rounded-lg shadow-md p-6 mb-6">
    ...
    <select onchange="window.location.href='/instrutor/frequencia_alunos.php?curso_id=' + this.value" class="w-full md:w-96 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500">
...
<?php if ($cursoSelecionado && empty($alunos)): ?>
            ...
            <div class="block bg-white rounded-lg shadow-md hover:shadow-xl transition overflow-hidden">
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <a href="/instrutor/perfil_aluno.php?aluno_id=<?= $aluno['id'] ?>&curso_id=<?= $cursoSelecionado ?>" class="bg-blue-600 hover:bg-blue-700 text-white text-center py-2 rounded-lg text-sm transition">
                            Frequência
                        </a>
                    </div>
                </div>
